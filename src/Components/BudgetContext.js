...
export const BudgetProvider = ({ children }) => {
  const categories = useMemo(
    () => ["Food", "Rent", "Transport", "Entertainment", "Utilities", "Others"],
    []
  );
  ...
  const addExpense = (product, price, category) => {
    ...
    const newTotal =
      totalExpenses +
      (expenseToEdit ? priceNumber - expenseToEdit.price : priceNumber);
    if (newTotal > budget) {
      ...
      setExpenses(
        expenses.map((expense) =>
          expense.id === expenseToEdit.id
            ? { ...expense, product, price: priceNumber, category }
            : expense
        )
      );
  ...
  const validateExpense = (product, price) => {
    ...
    if (priceNumber > remainingBalance)
      errors.price = "Price exceeds remaining budget";
